...
var startCmd = &cobra.Command{
	Use:   "start",
	Short: "start and synchronize ephemeral nodes from source to target",
	Long:  "start and synchronize ephemeral nodes from source to target",
...
func init() {
	...
	startCmd.Flags().StringVar(&startCfg.LogPath, "logPath", "./log/t_sync.log", "the path of the log file saved")
	...
	startCmd.Flags().StringVar(&startCfg.MonitorLogPath, "monitorLogPath", "./log/t_sync_info.log", "the path of the monitor log file saved")
	startCmd.Flags().StringVar(&startCfg.SummaryLogPath, "summaryLogPath", "./log/t_sync_stat.log", "the path of the summary log file saved")
...
func startCommandHandle(*cobra.Command, []string) {
	defer func() {
		if err := recover(); err != nil {
			fmt.Println("watcher encountered panic, err info: ", err)
			debug.PrintStack()
		}
	...
	processor, err := migration.NewProcessor(srcAddrInfo, dstAddrInfo, tunnel, processorOpt...)
	if err != nil {
	...
	tunnel.In(migration.Event{
		...
		Data:     []byte(""),
		Operator: migration.OptSideSource,
	})

	watcherOpts := []migration.WatcherManagerOption{
		migration.WatcherMediateOpt(migration.TSyncMediate),
	...
	syncOpts := []migration.SyncOption{
		migration.SyncMediateOpt(migration.TSyncMediate),

...
- all_sync：将持久化和临时节点从源ZK 正向同步到目标ZK
- t_sync：将临时节点从目标ZK 反向同步到源ZK

配置数据是持久化节点，迁移过程：

- 源ZK 的配置数据由all_sync 同步到目标ZK
- 对于接入目标ZK 的客户端，可以读取源ZK 的配置数据
- 在迁移过程中，只允许在源ZK 上修改配置数据

注册数据是临时节点，迁移过程：

- 接入源ZK 的客户端数据由all_sync 同步到目标ZK
- 对于接入目标ZK 的客户端，可以读取注册到源ZK 的客户端数据
- 接入目标ZK 的客户端数据由t_sync 同步到源ZK
- 对于还在源ZK 的客户端，可以读取注册到目标ZK 的客户端数据
